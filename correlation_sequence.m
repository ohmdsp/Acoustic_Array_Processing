...
% Filename:         correlation_sequencem
% Author:           D.R.Ohm   
% Software:         Matlab 7.01
% Rev.Date:         Sept. 15, 2005
%
...
% x       -- vector of data samples from channel x
% y       -- vector of data samples from channel y (CCS only)
...
if strcmp(bias,'biased')
    r = r/nx;                                   % eqs (5.13),(5.19) Marple text
else                                            % default is 'unbiased'
    r = r./(nx-abs(max_lag-(0:2*max_lag)))';    % eq (5.9)Marple text
end
%
